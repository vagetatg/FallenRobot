...
WRONG_FORMAT_TEXT = """Wrong ❌️ format...  eg. /u Img_url muzan-kibutsuji Demon-slayer 3

img_url character-name anime-name rarity-number

use rarity number accordingly rarity Map

rarity_map = (💫 Rare=1)  (🌿 Medium=2) (🦄 Legendary=3)(💮 Special Edition=4) (🔮 Limited Edition=5) (🎉 Festival=6) (🍂 Seasonal=7) (🎐 Celestial=8)"""
...
async def delete(update: Update, context: CallbackContext) -> None:
    ...
    try:
        args = context.args
        if len(args) != 1:
            await update.message.reply_text('Incorrect format... Please use: /dels ID')
            return
...
# Handlers
UPLOAD_HANDLER = CommandHandler('u', upload, block=False)
application.add_handler(UPLOAD_HANDLER)

DELETE_HANDLER = CommandHandler('dels', delete, block=False)
